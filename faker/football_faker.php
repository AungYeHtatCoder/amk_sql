...
try{
    ...
    $tableExists = $pdo->query("SHOW TABLES LIKE 'football'")->rowCount() > 0;
    if ($tableExists) {
        //Truncate the table
        $stmt = $pdo->prepare("truncate table football");
        $stmt->execute();
    } else {
        throw new Exception("The table 'football' does not exist in the database.");
    }

    //Insert the data
    $sql = 'INSERT INTO football (name) 
    VALUES (:name)';
